...
export const loadDatasets = async () => {
  ...
  const loadUndirected500 = async () => {
    const result = await fetch("https://raw.githubusercontent.com/graphology/graphology/master/src/communities-louvain/test/datasets/undirected1000.json")
    const undirected1000Data = await result.json();
    const graph = new UndirectedGraph();
    undirected1000Data.nodes.forEach((d: any) => {
      graph.addNode(d.id, d);
    });
    undirected1000Data.edges.forEach((d: any) => {
    ...
    return {
      desc:
        "Creates a undirected graph with 500 nodes.",
      [TestName.GRAPHOLOGY]: graph,
      [TestName.ANTV_ALGORITHM]: antvgraph,
      [TestName.ANTV_GRAPH_GPU]: antvgraph,
      [TestName.ANTV_GRAPH_WASM]: antvgraph,
    };
  };
  ...
  const loadRandomClusters = () => {
    ...
    return {
      desc:
        "Creates a graph with the desired number of nodes & edges and having a given number of clusters. Generated by Graphology Generators.",
      [TestName.GRAPHOLOGY]: graph,
      [TestName.ANTV_ALGORITHM]: antvgraph,
      [TestName.ANTV_GRAPH_GPU]: antvgraph,
      [TestName.ANTV_GRAPH_WASM]: antvgraph,
  ...
  datasets.undirected500 = undirected500;
